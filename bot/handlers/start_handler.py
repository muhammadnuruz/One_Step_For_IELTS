...
Ism-Familiya: {call.from_user.full_name}""", parse_mode='HTML')
    ...
    requests.post(url=f"http://127.0.0.1:8003/api/telegram-users/create/", data=data)
...
async def language_function_1(call: types.CallbackQuery, state: FSMContext):
    await state.finish()
    tg_user = json.loads(
        requests.get(url=f"http://127.0.0.1:8003/api/telegram-users/chat_id/{call.from_user.id}/").content)
    data = {
        "chat_id": str(call.from_user.id),
        "username": call.from_user.username,
        "full_name": call.from_user.full_name,
    ...
    requests.put(url=f"http://127.0.0.1:8003/api/telegram-users/update/{tg_user['id']}/", data=data)
    await call.message.delete()
    if call.data.split("_")[-1] == 'uz':
